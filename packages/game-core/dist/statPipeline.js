 ...
 */
export const resolveStats = (base, modifiers) => {
    // Stage 2: flat additive
    let flatSum = 0;
    for (const mod of modifiers) {
        if (mod.op === 'FLAT_ADD')
            flatSum = fpAdd(flatSum, mod.value);
    }
    let value = fpAdd(base, flatSum);
    // Stage 3: percent additive (stacked sum of percents, then applied once)
    let percentSum = 0;
    for (const mod of modifiers) {
        if (mod.op === 'PERCENT_ADD')
            percentSum = fpAdd(percentSum, mod.value);
    }
    if (percentSum !== 0) {
    ...
    for (const mod of modifiers) {
        if (mod.op === 'MIN_CAP') {
            if (!hasMinCap || mod.value > minCap) {
                minCap = mod.value;
                hasMinCap = true;
            }
        }
        if (mod.op === 'MAX_CAP') {
            if (!hasMaxCap || mod.value < maxCap) {
                maxCap = mod.value;
                hasMaxCap = true;
            }
        }
...
//# sourceMappingURL=statPipeline.js.map
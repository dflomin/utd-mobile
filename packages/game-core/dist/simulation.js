...
import { GAME_SCHEMA_VERSION } from '@utd/shared-types';
import { summarizeTelemetry } from '@utd/telemetry';
...
const createEnemyPosition = (pathProgress) => ({ x: Math.min(pathProgress, pathLength), y: 180 });
const inRange = (tower, enemy) => ((tower.x - enemy.x) ** 2) + ((tower.y - enemy.y) ** 2) <= tower.range ** 2;
export const createBuildTowerCommand = (applyAtTick = 0, clientSeq = 1) => ({
...
export const createStartWaveCommand = (applyAtTick = 0) => ({
    schemaVersion: GAME_SCHEMA_VERSION,
...
export const createSellTowerCommand = (applyAtTick = 1, clientSeq = 2) => ({
    schemaVersion: GAME_SCHEMA_VERSION,
...
export const createReplaceTowerCommand = (applyAtTick = 1, clientSeq = 2) => ({
    schemaVersion: GAME_SCHEMA_VERSION,
...
const reject = (state, code, commandId) => {
    state.telemetry.push({ type: 'commandRejected', sourceId: `${commandId}:${code}` });
    return code;
};
const validateCommand = (state, command) => {
    const config = createContentConfig();
    if (command.schemaVersion !== GAME_SCHEMA_VERSION)
        return reject(state, 'INVALID_SCHEMA_VERSION', command.commandId);
    if (!uuidPattern.test(command.commandId))
        return reject(state, 'INVALID_COMMAND_ID', command.commandId);
    if (command.sessionId !== sessionId || command.runId !== runId || command.playerId !== playerId)
        return reject(state, 'INVALID_MEMBERSHIP', command.commandId);
    if (command.clientSeq <= state.lastClientSeq)
        return reject(state, 'NON_MONOTONIC_CLIENT_SEQ', command.commandId);
    if (command.applyAtTick < state.tick || command.applyAtTick > state.tick + config.economy.schedulingWindowTicks)
        return reject(state, 'INVALID_SCHEDULE_WINDOW', command.commandId);
    if (state.acceptedCommandIds.has(command.commandId))
        return 'DUPLICATE_COMMAND';
    if (state.lives <= 0)
        return reject(state, 'GAME_OVER', command.commandId);
    if (command.type === 'START_WAVE' && !config.waves.some((wave) => wave.id == command.payload.waveId))
        return reject(state, 'UNKNOWN_WAVE', command.commandId);
    ...
    if (command.type === 'SELL_TOWER') {
        ...
        if (!existing || existing.towerDefId !== command.payload.towerDefId)
            return reject(state, 'UNKNOWN_TOWER', command.commandId);
        if (existing.instanceId !== command.payload.towerInstanceId)
            return reject(state, 'UNKNOWN_TOWER_INSTANCE', command.commandId);
        return null;
    }
    if (command.type === 'REPLACE_TOWER') {
        const existing = state.towers.find((entry) => entry.id === command.payload.targetTowerEntityId);
        if (!existing || existing.towerDefId !== command.payload.targetTowerDefId)
            return reject(state, 'UNKNOWN_TOWER', command.commandId);
        if (existing.instanceId !== command.payload.targetTowerInstanceId)
            return reject(state, 'UNKNOWN_TOWER_INSTANCE', command.commandId);
        ...
        if (!targetTower || !replacementTower)
            return reject(state, 'UNKNOWN_TOWER', command.commandId);
        if (!replacementStack || replacementStack.count <= 0)
            return reject(state, 'BLUEPRINT_NOT_OWNED', command.commandId);
        if (replacementStack.towerDefId !== replacementTower.id)
            return reject(state, 'BLUEPRINT_MISMATCH', command.commandId);
        if (targetTower.element !== replacementTower.element)
            return reject(state, 'REPLACEMENT_ELEMENT_MISMATCH', command.commandId);
        if (state.gold < replacementTower.cost)
            return reject(state, 'INSUFFICIENT_GOLD', command.commandId);
        return null;
    }
    ...
    if (!tower)
        return reject(state, 'UNKNOWN_TOWER', command.commandId);
    if (!slot)
        return reject(state, 'UNKNOWN_SLOT', command.commandId);
    if (!stack || stack.count <= 0)
        return reject(state, 'BLUEPRINT_NOT_OWNED', command.commandId);
    if (stack.towerDefId !== tower.id)
        return reject(state, 'BLUEPRINT_MISMATCH', command.commandId);
    if (state.gold < tower.cost)
        return reject(state, 'INSUFFICIENT_GOLD', command.commandId);
    if (state.towers.some((entry) => entry.slotId === slot.id))
        return reject(state, 'SLOT_OCCUPIED', command.commandId);
    return null;
};
const applyCommand = (state, command) => {
    const config = createContentConfig();
    const rejection = validateCommand(state, command);
    if (rejection === 'DUPLICATE_COMMAND')
        return;
    if (rejection)
        return;
...
const spawnWaveEnemies = (state) => {
    if (!state.waveStarted || state.waveStartTick === null)
        return;
    const config = createContentConfig();
...
const runToTick = (options, stopAtTick) => {
    const config = createContentConfig();
    ...
    while (state.tick < stopAtTick && state.lives > 0) {
        ...
        if (state.spawnedEnemies >= wave.count && state.enemies.length === 0)
            break;
    }
...
export const runScriptedSimulation = (options) => runToTick(options, options.maxTicks).summary;
export const createRenderSnapshot = (options = {}) => {

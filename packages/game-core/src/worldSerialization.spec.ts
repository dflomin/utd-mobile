...
describe('serializeStores / deserializeStores', () => {
  it('round-trips an empty world', () => {
    ...
    const hp = new ComponentStore<number>();
    const original = serializeStores(alloc, { hp });
    const { alloc: alloc2, stores } = deserializeStores(original, { hp: () => new ComponentStore<number>() });
  ...
  it('round-trips entities with components', () => {
    ...
    const hp = new ComponentStore<{ current: number; max: number }>();
    ...
    const { stores } = deserializeStores(snapshot, {
      hp: () => new ComponentStore<{ current: number; max: number }>(),
    });
  ...
  it('snapshot is JSON-serializable (plain object)', () => {
    ...
    const pos = new ComponentStore<{ x: number; y: number }>();
    ...
    const { stores } = deserializeStores(parsed, {
      pos: () => new ComponentStore<{ x: number; y: number }>(),
    });
